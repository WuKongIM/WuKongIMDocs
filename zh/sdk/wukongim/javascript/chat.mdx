...
`需要实现上传文件的数据源` 参考：[上传文件数据源](/zh/sdk/wukongim/javascript/datasource#文件上传数据源)
...
参考自定义消息: [自定义消息](/zh/sdk/wukongim/javascript/advance#自定义消息)
...
当消息发送后，通过 `addMessageStatusListener` 监听返回的 `SendackPacket` 中包含 `reasonCode`。以下是各状态码的说明：
...
`需要实现同步频道消息数据源` 参考：[同步频道消息数据源](/zh/sdk/wukongim/javascript/datasource#同步频道消息数据源)
